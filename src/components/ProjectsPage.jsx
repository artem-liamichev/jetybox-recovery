...
function ProjectsPage() {
  ...
  return (
    <main className="min-h-screen bg-[#2c2c2c] px-4 pb-8 pt-8 md:px-0 md:pb-0">
      <section className="pt-[34px] md:mx-auto md:max-w-[940px] md:pt-[64px]">
        ...
        <div className="mt-5 grid gap-4 md:mt-0 md:grid-cols-2 md:gap-x-[18px] md:gap-y-[10px]">
          {projects.map((project) => (
            <button
              key={project.titleKey}
              type="button"
              onClick={() => navigateTo(`/projects/${project.id}`)}
              className="relative z-10 mr-9 min-h-[250px] w-full min-w-[300px] overflow-hidden rounded-[2px] border border-white/10 bg-[#1f1f1f] md:mr-0 md:min-h-0 md:min-w-0"
            >
              <img
                src={project.image}
                alt={t(project.titleKey)}
                className="h-full w-full object-cover md:aspect-[394/190]"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[rgba(10,10,10,0.88)] via-[rgba(10,10,10,0.2)] to-transparent" />
              <div className="absolute inset-x-0 bottom-0 flex flex-col gap-1 p-2.5 text-left md:p-[10px]">
                <span className="text-left font-[Helvetica] text-xl font-normal leading-6 text-white/70 md:text-xl md:leading-6">
                  {t(project.categoryKey)}
                </span>
                <h2 className="text-left font-[Helvetica] text-2xl font-bold leading-[1.05] text-white md:text-[22px]">
                  {t(project.titleKey)}
                </h2>
              </div>
            </button>
